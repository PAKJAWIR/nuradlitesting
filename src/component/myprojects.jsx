...
function MyProjects() {
  ...
  // --- 🔥 Tambahkan useEffect untuk reset visibleCount saat filter ganti
  useEffect(() => {
    ...
    <div className="flex flex-col gap-5 px-4 lg:px-5 w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex flex-col">
          ...
          <p className="text-warna3 text-sm mt-1">check out my latest projects!</p>
        </div>

        <div className="flex flex-wrap items-start justify-start text-center lg:justify-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 w-full min-h-[300px]">
        {visibleProjects.length > 0 ? (
          visibleProjects.map((proj) => (
            <Tilt key={proj.id} tiltMaxAngleX={15} tiltMaxAngleY={15} scale={1.05} transitionSpeed={400} glareEnable={true} glareMaxOpacity={0.1} glarePosition="all" className="transition-transform duration-300 hover:z-10">
              <div className="flex flex-col border border-solid border-warna4 items-center rounded-md text-center bg-warna5 transition-all duration-300 hover:shadow-[0_0_20px_theme(colors.warna4)] hover:border-warna4">
          ...
          <div className="col-span-full flex items-center justify-center text-warna3 text-sm">No projects found.</div>
        )}
      </div>
      ...
      {filteredProjects.length > 3 && (
        <div className="flex justify-center mt-1">
          <button onClick={handleLoadMore} className="px-6 py-2 text-sm rounded-md border border-warna4 text-warna3 hover:bg-warna4 hover:text-warna1 hover:scale-105 transition-all">
