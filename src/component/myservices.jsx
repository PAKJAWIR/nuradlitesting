...
export function MyServices() {
  ...
  return (
    <div className="flex flex-col justify-center max-w-5xl gap-3">
      <Title1 text="my-services" />
      <div onMouseEnter={textEnter} onMouseLeave={textLeave} className="grid grid-cols-1 md:grid-cols-2 gap-5 w-fit">
...
export function MyServices2() {
  const cardClass =
    "border border-solid border-warna4 w-full h-full flex flex-col items-center p-5 rounded-md text-center bg-warna5 transition-all duration-300 hover:shadow-[0_0_20px_theme(colors.warna4)] hover:border-warna4";
  const titleClass = "text-warna1 whitespace-nowrap text-md font-bold mt-2";
  const descriptionClass = "text-warna3 text-center leading-relaxed tracking-wide text-xs p-1";
  const { textEnter, textLeave } = useCursor();

  return (
    <div className="flex flex-col justify-center max-w-6xl p-10 lg:p-10 gap-3">
      <Title1 text="my-services" />
      <div onMouseEnter={textEnter} onMouseLeave={textLeave} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 w-full">
        <Tilt tiltMaxAngleX={15} tiltMaxAngleY={15} scale={1.05} transitionSpeed={400}>
          <div className={cardClass}>
            <FaCode className="text-warna4 text-lg" />
            <h3 className={titleClass}>web-building</h3>
